<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <a href="{{ route('prestations.index') }}" wire:navigate class="text-sm font-medium text-ninich-rose hover:underline">&larr; Retour au catalogue</a>

    <div class="text-center mt-4 mb-8">
        <h1 class="font-serif text-4xl font-bold">Réserver votre rendez-vous</h1>
        <p class="text-ninich-muted mt-2">Complétez les étapes ci-dessus pour confirmer votre créneau.</p>
    </div>
                    ...
                    @foreach ($this->prestataires as $p)
                        <button type="button" wire:click="$set('prestataire_id', '{{ $p->id }}')"
                            ...
                            <div class="flex-1">
                                ...
                                @if($p->bio)<span class="text-xs text-ninich-muted">{{ $p->bio }}</span>@endif
                            </div>
                            @if ((int)$prestataire_id === $p->id)
                                <span class="w-6 h-6 rounded-full bg-ninich-rose text-white flex items-center justify-center text-sm">✓</span>
                            @endif
                        </button>
                    @endforeach
                </div>
                @error('prestataire_id') <p class="text-sm text-red-600 mt-2">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="bg-white border border-ninich-line rounded-[22px] p-7 shadow-ninich">
                ...
                @else
                    ...
                    <div class="flex gap-5 mt-4 text-xs text-ninich-muted">
                        <span class="inline-flex items-center gap-1.5"><i class="inline-block w-3 h-3 rounded bg-ninich-rose"></i>Sélectionné</span>
                        <span class="inline-flex items-center gap-1.5"><i class="inline-block w-3 h-3 rounded bg-white border border-ninich-line"></i>Disponible</span>
                        <span class="inline-flex items-center gap-1.5"><i class="inline-block w-3 h-3 rounded bg-ninich-blush-2"></i>Indisponible</span>
                    </div>
                @endif
                @error('heure') <p class="text-sm text-red-600 mt-2">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="bg-white border border-ninich-line rounded-[22px] overflow-hidden shadow-ninich">
                ...
                <div class="p-6">
                    ...
                    <div class="my-5 border-y border-dashed border-ninich-line py-4 space-y-3 text-sm">
                        <div class="flex items-center justify-between"><span class="text-ninich-muted">👩‍🔬 Experte</span><b>{{ $prestataireChoisi->name ?? '—' }}</b></div>
                        <div class="flex items-center justify-between"><span class="text-ninich-muted">📅 Date</span><b>{{ $date ? \Carbon\Carbon::parse($date)->translatedFormat('D d M Y') : '—' }}</b></div>
                        <div class="flex items-center justify-between"><span class="text-ninich-muted">🕒 Heure</span><b>{{ $heure ?: '—' }}</b></div>
                        <div class="flex items-center justify-between"><span class="text-ninich-muted">⏱ Durée</span><b>{{ $prestation->duree }} min</b></div>
                    </div>
                    <div class="flex items-baseline justify-between mb-5">
                        <span class="text-ninich-muted text-sm">Total</span>
                        <b class="font-serif text-3xl text-ninich-rose-dark">{{ number_format($prestation->prix, 2, ',', ' ') }} €</b>
                    </div>
                    <button type="submit"
                    ...
                    <p class="text-xs text-ninich-muted text-center mt-3">Vous recevrez une confirmation. Annulation gratuite jusqu'à 24 h avant.</p>
                </div>
            </div>
        </aside>
    </form>
